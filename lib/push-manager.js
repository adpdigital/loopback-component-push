...
PushManager.prototype.on('application-update', function(appId) {
  var cacheApp = this.applicationsCache.get(appId);
  if (cacheApp && cacheApp['ios']) {
    cacheApp['ios'].emit('shutdown-connections')
  }
  this.applicationsCache.del(appId)
  this.asyncApplicationsCache.reset()
})
