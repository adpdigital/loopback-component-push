...
PushManager.prototype.configureProvider = function (deviceType, pushSettings, cb) {
  ...
  try {
    ...
    provider.on('devicesGone', function (deviceTokens, originRequest) {
      this.emit('devicesGone', deviceTokens, deviceType, pushSettings.appId, originRequest);
    }.bind(this));

    provider.on('error', function (err) {
      if (err.message.indexOf('XMPP authentication') > -1) {
    ...
    provider.on('transmitted', function (notif, result, originRequest) {
      this.emit('transmitted', notif, result, pushSettings.appId, originRequest);
    }.bind(this));

    provider.on('transmissionError', function (err, notification, recipient, originRequest) {
      this.emit('transmissionError', err, notification, recipient, pushSettings.appId, originRequest);
    }.bind(this));
...
PushManager.prototype.notify = function (installation, notification, cb) {
  ...
  this.configureApplication(
    ...
    function (err, provider) {
      ...
      provider.pushNotification(notification, deviceToken, installation);
      cb();
    }
  );
};
