...
PushManager.prototype.configureProvider = function (deviceType, pushSettings, cb) {
  ...
  try {
    var provider = new Provider(pushSettings, this, this.Application.app);
...
/**
 * Adds the given trackId to cancelled push list.
 * @param {String} trackId
 * @return {Set} The new Set of cancelled pushes
 */
PushManager.prototype.addToCancelled = function(trackId) {
  return trackId && CANCELLED_PUSH.add(trackId);
};

/**
 * Check if given notification is cancelled or not.
 * @param {Notification} notification The notification to check.
 * @return {Boolean} Whether it's cancelled or not.
 */
PushManager.prototype.isCancelled = function(notification) {
  return notification && notification.trackId && CANCELLED_PUSH.has(notification.trackId);
};
...
PushManager.prototype.notify = function (installation, notification, cb) {
  ...
  if (this.isCancelled(notification)) {
    // console.log('@@@@@ cancelled', notification.trackId)
    return cb();
  }
