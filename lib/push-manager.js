...
PushManager.prototype.configureProvider = function (deviceType, pushSettings, cb) {
  var Provider = PushManager.providers[pushSettings[deviceType+'Provider'] || deviceType];
  if (!Provider) {
  ...
  try {
    ...
    provider.on('devicesGone', function (deviceTokens) {
      this.emit('devicesGone', deviceTokens, deviceType, pushSettings.appId);
    }.bind(this));
    ...
    provider.on('transmitted', function (notif, result) {
      this.emit('transmitted', notif, result, pushSettings.appId);
    }.bind(this));

    provider.on('transmissionError', function (err, notification, recipient) {
      this.emit('transmissionError', err, notification, recipient, pushSettings.appId);
    }.bind(this));
