...
function GcmProvider(pushManager, app, pushSettings) {
  ...
  app.emit('shutdownable', {
    name: `FCMQueue-${this.appId}-${process.pid}`,
    service: this
  })
  ...
  app.on('shutdown', () => {
    ...
    if (this._queue.idle()) {
      this.app.log.info({queue: `FCMQueue-${this.appId}-${process.pid}`, idle: this._queue.idle()}, 'shutdown:FCMQueue already empty')
      app.emit('shutdownReady', {
        name: `FCMQueue-${this.appId}-${process.pid}`,
        service: this
      })
    } else {
      this.app.log.info({queue: `FCMQueue-${this.appId}-${process.pid}`, idle: this._queue.idle()}, 'shutdown:FCMQueue wait to drain')
      this._queue.drain = () => {
        if (this.shuttedDown) {
          this.app.log.info({queue: `FCMQueue-${this.appId}-${process.pid}`, idle: this._queue.idle()}, 'shutdown:FCMQueue empty')
          app.emit('shutdownReady', {
            name: `FCMQueue-${this.appId}-${process.pid}`,
            service: this
          })
  ...
  setInterval(function(){
    ...
    const rate = Math.round(Math.abs(this.currentLength - this._queue.length()) / 5)
    this.currentLength = this._queue.length()
    if (rate !== 0) {
      this.app.log.info({length: this.currentLength, rate, appId: this.appId}, 'GCM-HTTP queue msg/sec')
    }
  }.bind(this), 5000)
...
GcmProvider.prototype._setupPushConnection = function(options) {
  this.app.log.info({appId: this.appId, concurrency: options.concurrency}, 'Setting up GCM-HTTP')
...
GcmProvider.prototype.sender = function (item, cb) {
  var self = this
