...
GcmProvider.prototype.sender = function (item, callback) {
  ...
  debug('Pushing FCM to [%s] app=[%s] ', item.deviceToken, item.note);

  this._connection.send(item.note, item.deviceToken, 2, function (err, result) {
    if (err) {
      console.error('FCM-HTTP Cannot send message ', err, item.note, result);

      self.app.statsD && self.app.statsD.increment(['app', self.appId, 'gcm', 'fail'].join('.'));

      item.notification.android = true

      // item.notification.message_id = result.getMessageId();
      // TODO recipient is multi tokens? How should be item.deviceToken handles upward?
      // TODO what is result.getErrorDescription()?
      // TODO how is message_id gained via result.getMessageId()?

      self.emit('transmissionError', new Error(err), item.notification, item.deviceToken, result.getErrorDescription());
    ...
    console.log('FCM-HTTP result: %j', result);
      ...
      .map(function(value, index){
        if (value.error) {
          ...
          if (code === 'NotRegistered' || code === 'InvalidRegistration') {
            devicesGoneRegistrationIds.push(item.deviceToken[index]);
          } else if (code) {
            console.error('FCM-HTTP send error code: ' + (code || 'Unknown') + ', deviceToken: ' + item.deviceToken[index]);
          }
        } else {
          self.app.statsD && self.app.statsD.increment(['app', self.appId, 'gcm', 'sent'].join('.'));
          self.emit("transmitted", item.note, item.notification.message_id);
        }
      })
