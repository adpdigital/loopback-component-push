...
function GcmProvider(pushSettings, pushManager, app) {
...
GcmProvider.prototype.sender = function (item, callback) {
  var self = this
  if (this.pushManager.isCancelled(item.notification)) {
    this.app.log.warn({trackId: item.notification.trackId}, 'ignoring cancelled push')
    return callback()
  }
