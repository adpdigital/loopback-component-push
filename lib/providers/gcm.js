...
function GcmProvider(pushManager, app, pushSettings) {
  ...
  settings.concurrency = settings.concurrency || 100
...
GcmProvider.prototype._setupPushConnection = function(options) {
  this.app.log.info({appId: this.appId, serverApiKey: options.serverApiKey, concurrency: options.concurrency}, 'Setting up GCM-HTTP')
