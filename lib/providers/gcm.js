...
GcmProvider.prototype.sender = function (item, callback) {
  ...
  this._connection.send(item.note, item.deviceToken, 2, function (err, result) {
    if (err) {
      ...
      self.emit('transmissionError', new Error(err), item.notification, recipients, err);
    ...
    result.results
      .map(function(value, index){
        if (value.error) {
          ...
          console.error('FCM-HTTP send error code: ' + (code || 'Unknown') + ', deviceToken: ' + item.deviceToken.registrationTokens[index]);
          self.emit('transmissionError', new Error(code), item.notification, item.deviceToken.registrationTokens[index], code);
        } else {
          self.app.statsD && self.app.statsD.increment(['app', self.appId, 'gcm', 'sent'].join('.'));
          self.emit("transmitted", item.note, item.notification.messageId || value.message_id, item.deviceToken.registrationTokens);
        }
      })
