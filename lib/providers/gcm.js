...
GcmProvider.prototype.pushNotification = function(notification, deviceToken, installation, cb) {
  ...
  if (self.app.dedup) { // todo only dedup for available trackId, messageId pairs
    // todo map on registrationIds
    self.app.dedup(self.appId, `${notification.trackId}-${message.messageId}`, deviceToken).then((alreadyExists) => {
      if (alreadyExists) {
        cb && cb()
        // todo pass up notif deduped event
      } else {
...
GcmProvider.prototype.sender = function (item, callback) {
  ...
  this._connection.send(item.note, item.deviceToken, {retries: 3, backoff: 3000}, function (err, result) {
    self.app.log.debug({err, result}, '{FCM-SENT}')
    ...
    if (err) {
      ...
      recipients.forEach(function (token) {
        self.app.log.debug({notification: item.notification, recipients, err}, '[FCM-MODE] transmissionError before send')
        if (err === 401) {
    ...
    self.app.log.debug({result}, 'FCM-HTTP result')
    ...
    result.results
      .map(function(value, index){
        if (value.error) {
          ...
          self.app.log.debug({notification: item.notification, registrationTokens: item.deviceToken.registrationTokens[index]}, '[FCM-MODE] transmissionError after send')
          ...
          self.app.log.debug({note: item.note, messageId: item.notification.messageId || value.message_id, notification: item.notification}, '[FCM-MODE] transmitted')
