...
GcmProvider.prototype.pushNotification = function(notification, deviceToken, installation) {
  var self = this;
  var to;
  var length = 1
...
GcmProvider.prototype.sender = function (item, callback) {
  ...
  this._connection.send(item.note, item.deviceToken, 2, function (err, result) {
    ...
    if (err) {
      ...
      // TODO recipient is multi tokens? How should be item.deviceToken handles upward?
      ...
      recipients.forEach(function (token) {
        debug('>>>>>>>>>>> [FCM-MODE] transmissionError', item.notification, recipients, err)
        self.emit('transmissionError', new Error(err), item.notification, token);
      })
      ...
      .map(function(value, index){
        if (value.error) {
          ...
          if (code === 'NotRegistered' || code === 'InvalidRegistration') {
            debug('>>>>>>>>>>>>> notRegistered or InvalidRegistration, item pushed to devicesGoneRegistrationIds')
            devicesGoneRegistrationIds.push(item.deviceToken.registrationTokens[index]);
          }
          debug('>>>>>>>>>>> [FCM-MODE][MAP] transmissionError', item.notification, item.deviceToken.registrationTokens[index])
          self.emit('transmissionError', new Error(code), item.notification, item.deviceToken.registrationTokens[index]);
        } else {
          self.app.statsD && self.app.statsD.increment(['app', self.appId, 'gcm', 'sent'].join('.'));
          debug('>>>>>>>>>>> [FCM-MODE] transmitted', item.note, item.notification.messageId || value.message_id)
          self.emit("transmitted", item.note, item.notification.messageId || value.message_id);
        }
      })
