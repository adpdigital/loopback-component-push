...
function WebProvider(pushSettings) {
  ...
  if (webSettings.mailto) {
    this._pushOptions.mailto = webSettings.mailto || 'mailto:chabokpush@adpdigital.com'
  }

  if (webSettings.publicKey) {
    this._pushOptions.publicKey = webSettings.publicKey || vapidKeys.publicKey
  }
  if (webSettings.privateKey) {
    this._pushOptions.privateKey = webSettings.privateKey || vapidKeys.privateKey
  }
  if (webSettings.production) {
    this._pushOptions.production = webSettings.production
  }
  if (webSettings.bundleId) {
    this._pushOptions.bundleId = webSettings.bundleId
  }
  if (webSettings.connectionRetryLimit) {
    this._pushOptions.connectionRetryLimit = webSettings.connectionRetryLimit
  }
  if (webSettings.heartBeat) {
    this._pushOptions.heartBeat = webSettings.heartBeat
  }
