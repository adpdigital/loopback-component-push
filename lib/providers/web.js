...
function WebProvider(pushManager, app, pushSettings) {
  ...
  this.app.log.info({options: this._pushOptions, appId: this.appId}, 'Setting up Web-Push')
}
...
WebProvider.prototype.pushNotification = function (notification, deviceToken, installation, cb) {
  ...
  if (notification.mediaUrl) notif.image = notification.mediaUrl
  ...
  if (notification.notifDelivery) notif.notifDelivery = notification.notifDelivery
  if (notification.disableLocalNotif) notif.disableLocalNotif = notification.disableLocalNotif
  if (notification.fromChabok) notif.fromChabok = notification.fromChabok

  this.app.log.debug({appId: installation.appId, subscription, notif}, 'Sending web notification')
  ...
  if (self.pushManager && self.pushManager.isCancelled(notification)) {
  ...
  webpush
    ...
    .then(function(success) {
      ...
      self.app.log.debug({success}, 'WebPush Success')
    ...
    .catch(function(error) {
      ...
      self.app.log.error({appId: installation.appId, error}, 'WebPush Error')
