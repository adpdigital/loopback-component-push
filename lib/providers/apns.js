...
function ApnsProvider(pushSettings, app) {
  ...
  setInterval(function(){
    console.log('APNS queue[%s, %s]: ', this.appId, this._queue.concurrency, this._queue.length())
  }.bind(this), 5000)
...
ApnsProvider.prototype.pushNotification = function (notification, deviceToken) {
  ...
  this.app.statsD.increment(['app', this.appId, 'apn', 'request'].join('.'));
  ...
  this._queue.push({
    ...
    if(err) {
      self.emit('error', err);
      self.app.statsD.increment(['app', self.appId, 'apn', 'error'].join('.'));
      return console.error('WOW! what the hell out of apn? ', err);
    }
    if (!result && notification.statsClbk) {
      self.app.statsD.increment(['app', self.appId, 'apn', 'sent'].join('.'));
      return
    }
    ...
    result.failed.map(function (e) {
      if (e.error) {
        self.app.statsD.increment(['app', self.appId, 'apn', 'error'].join('.'));
        console.error('apn error ', JSON.stringify(e));
        self.emit('error', e);

      } else if (e.status && e.response) {
        self.app.statsD.increment(['app', self.appId, 'apn', 'fail'].join('.'));
        console.error('apn failed ', JSON.stringify(e));
...
ApnsProvider.prototype.sender = function (item, callback) {
  ...
  this.app.statsD.increment(['app', this.appId, 'apn', 'submit'].join('.'));
...
ApnsProvider.prototype.addConnection = function (i) {
  console.log(
    'Setting up APNS connection [%s of %s] with concurrency %s for %s[%s]',
    i+1, this.maxConnections, this.concurrency, this.appId, this.apnSettings.bundleId
  )
