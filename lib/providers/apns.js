...
function ApnsProvider(pushSettings) {
  ...
  var concurrency = apnSettings.concurrency || 300;
  this.maxConnections = apnSettings.maxConnections || 1;
  this._connections = [];
  this._current = 0;

  this._queue = queue(this.worker.bind(this), this.maxConnections * concurrency);

  for(var i = 0; i < this.maxConnections; i++) {
    const apnClient = new apn.Provider(this._pushOptions)
    console.log('Creating new apn connection [%s of %s] for ', i, this.maxConnections, apnSettings.bundleId, apnClient)
    this._connections.push(apnClient);
  }
}
...
ApnsProvider.prototype.worker = function (item, callback) {
  debug('Pushing notification to %j:', item.deviceToken, item.note);
