...
function ApnsProvider(pushSettings) {
  ...
  this._connection = new apn.Provider(this._pushOptions);
}
...
ApnsProvider.prototype.pushNotification = function (notification, deviceToken) {
  ...
  note.locKey = notification.locKey;
  note.locArgs = notification.locArgs;
  note.title = notification.title;
  note.titleLocKey = notification.titleLocKey;
  note.titleLocArgs = notification.titleLocArgs;
  note.action = notification.action;
  note.actionLocKey = notification.actionLocKey;
  note.launchImage = notification.launchImage;
  ...
  this._connection.send(note, deviceToken).then(function (result) {
    ...
    result.sent.map(function (s) {
      // s.device
      debug('apn success ', JSON.stringify(s), notification);
    ...
    result.failed.map(function (e) {
      if (e.error) {
        ...
        self.emit('transmissionError', err, notification, e.device);
