...
function ApnsProvider(pushSettings) {
  ...
  var concurrency = apnSettings.concurrency || 500;
  ...
  for(var i = 0; i < this.maxConnections; i++) {
    ...
    console.log('Creating new apn connection [%s of %s] with concurrency %s for ', i, this.maxConnections, this.concurrency, apnSettings.bundleId)
