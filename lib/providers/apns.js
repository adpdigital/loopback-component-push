...
function ApnsProvider(pushManager, app, pushSettings) {
  ...
  app.emit('shutdownable', {
    name: `APNQueue-${this.appId}-${process.pid}`,
    service: this
  })
  ...
  app.on('shutdown', () => {
    ...
    if (this._queue.idle()) {
      this.app.log.info({queue: `APNQueue-${this.appId}-${process.pid}`, idle: this._queue.idle()}, 'shutdown:APNQueue already empty')
      app.emit('shutdownReady', {
        name: `APNQueue-${this.appId}-${process.pid}`,
        service: this
      })
    } else {
      this.app.log.info({queue: `APNQueue-${this.appId}-${process.pid}`, idle: this._queue.idle()}, 'shutdown:APNQueue wait to drain')
      this._queue.drain = () => {
        if (this.shuttedDown) {
          this.app.log.info({queue: `APNQueue-${this.appId}-${process.pid}`, idle: this._queue.idle()}, 'shutdown:APNQueue empty')
          app.emit('shutdownReady', {
            name: `APNQueue-${this.appId}-${process.pid}`,
            service: this
          })
  ...
  setInterval(function () {
    ...
    if (rate !== 0) {
      this.app.log.info({length: this.currentLength, rate, appId: this.appId}, 'APN queue msg/sec')
    }
  }.bind(this), 5000)
