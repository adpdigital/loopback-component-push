...
GcmCcsProvider.prototype._setupPushConnection = function(options) {
  console.log('Using GCM-XCS Server API key %s & senderId %s', options.serverApiKey, options.senderId);
  ...
  this._connection.on('error', function(err){
    console.error('GCM-XCS Error ',err);
...
GcmCcsProvider.prototype.sender = function (item, callback) {
  ...
  this._connection.sendNoRetry(item.note, item.deviceToken, function(result){
    if (result.getError()) {
      ...
      console.log('Sent GCM to "', item.notification.deviceId, '" message_id "', result.getMessageId() + '"');
