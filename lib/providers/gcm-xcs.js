...
function GcmCcsProvider(pushSettings, app) {
  ...
  setInterval(function(){
    console.log('GCM queue[%s, %s]: ', this.appId, this._queue.concurrency, this._queue.length())
  }.bind(this), 5000)
...
GcmCcsProvider.prototype.pushNotification = function(notification, deviceToken) {
  ...
  targets.map( function(token){
    ...
    self.app.statsD.increment(['app', self.appId, 'gcm', 'request'].join('.'));
    self._queue.push({
      note: xcs_message,
...
GcmCcsProvider.prototype.sender = function (item, callback) {
  ...
  this.app.statsD.increment(['app', this.appId, 'gcm', 'submit'].join('.'));
